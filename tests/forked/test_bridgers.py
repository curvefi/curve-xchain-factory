...
def test_optimism_bridger(alice, crv_token, OptimismBridger):
    ...
    bridger = OptimismBridger.deploy(l2_crv, optimism_bridge, {"from": alice})
    ...
    tx = bridger.bridge(crv_token, alice, 10**18 // 2, {"from": alice, "value": bridger.cost()})

    assert crv_token.balanceOf(optimism_bridge) == balance_before + 10**18 // 2
    assert "ERC20DepositInitiated" in tx.events
    assert tx.events["ERC20BridgeInitiated"].values() == [
        crv_token,
        l2_crv,
        bridger,
        alice,
